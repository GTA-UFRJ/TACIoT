#!/bin/bash
results_path="./benchmark/throughput.txt"
...
for throughput in 2000 4000 6000 8000 10000 12000 14000 16000;
do
    echo -e "\n------ $throughput req/sec------" &>> $results_path
    rm database/sample
    touch database/sample
    ./Server i > /dev/null 2>&1 &
    sleep 1
    $wrk2path -t1 -c1 -d30s -R$(($throughput)) $publish_time_command &>> $results_path
    ps -ef | grep ./Server | grep -v grep | awk '{print $2}' | xargs kill >> /dev/null 2>&1
done
...
do
    ...
    ./Server i > /dev/null 2>&1 &
    sleep 1
    $wrk2path -t1 -c1 -d30s -R$(($throughput)) $query_time_command &>> $results_path
    ps -ef | grep ./Server | grep -v grep | awk '{print $2}' | xargs kill >> /dev/null 2>&1
done
rm database/sample
