...
static sgx_status_t SGX_CDECL server_enclave_ocall_print_secret(void* pms)
{
	ms_ocall_print_secret_t* ms = SGX_CAST(ms_ocall_print_secret_t*, pms);
	ocall_print_secret(ms->ms_secret, ms->ms_secret_size);
...
static const struct {
	size_t nr_ocall;
	void * table[1];
} ocall_table_server_enclave = {
	1,
	{
		(void*)server_enclave_ocall_print_secret,
	}
};
sgx_status_t enclave_init_ra(sgx_enclave_id_t eid, sgx_status_t* retval, int b_pse, sgx_ra_context_t* p_context, sgx_ec256_public_t* client_pk)
{
	sgx_status_t status;
	ms_enclave_init_ra_t ms;
	ms.ms_b_pse = b_pse;
	ms.ms_p_context = p_context;
	ms.ms_client_pk = client_pk;
	status = sgx_ecall(eid, 0, &ocall_table_server_enclave, &ms);
...
sgx_status_t enclave_ra_close(sgx_enclave_id_t eid, sgx_status_t* retval, sgx_ra_context_t context)
{
	...
	status = sgx_ecall(eid, 1, &ocall_table_server_enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
...
sgx_status_t verify_att_result_mac(sgx_enclave_id_t eid, sgx_status_t* retval, sgx_ra_context_t context, uint8_t* message, size_t message_size, uint8_t* mac, size_t mac_size)
{
	...
	status = sgx_ecall(eid, 2, &ocall_table_server_enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
...
sgx_status_t put_secret_data(sgx_enclave_id_t eid, sgx_status_t* retval, sgx_ra_context_t context, uint8_t* p_secret, uint32_t secret_size, uint8_t* gcm_mac)
{
	...
	status = sgx_ecall(eid, 3, &ocall_table_server_enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
...
sgx_status_t sgx_ra_get_ga(sgx_enclave_id_t eid, sgx_status_t* retval, sgx_ra_context_t context, sgx_ec256_public_t* g_a)
{
	...
	ms.ms_context = context;
	ms.ms_g_a = g_a;
	status = sgx_ecall(eid, 4, &ocall_table_server_enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
...
sgx_status_t sgx_ra_proc_msg2_trusted(sgx_enclave_id_t eid, sgx_status_t* retval, sgx_ra_context_t context, const sgx_ra_msg2_t* p_msg2, const sgx_target_info_t* p_qe_target, sgx_report_t* p_report, sgx_quote_nonce_t* p_nonce)
{
	...
	status = sgx_ecall(eid, 5, &ocall_table_server_enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
...
sgx_status_t sgx_ra_get_msg3_trusted(sgx_enclave_id_t eid, sgx_status_t* retval, sgx_ra_context_t context, uint32_t quote_size, sgx_report_t* qe_report, sgx_ra_msg3_t* p_msg3, uint32_t msg3_size)
{
	...
	status = sgx_ecall(eid, 6, &ocall_table_server_enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
